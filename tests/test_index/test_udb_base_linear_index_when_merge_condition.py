...
def test_should_override_gt_value():
    q = UdbBaseLinearIndex.merge_condition({'a': 1, 'b': {'$gt': 1}}, {'b': {'$gt': 2}, 'c': 4})
...
def test_should_override_lt_value():
    q = UdbBaseLinearIndex.merge_condition({'a': 1, 'b': {'$lt': 2}}, {'b': {'$lt': 1}, 'c': 4})

    assert q == {'a': 1, 'b': {'$lt': 1}, 'c': 4}
...
def test_should_override_lte_value():
    q = UdbBaseLinearIndex.merge_condition({'a': 1, 'b': {'$lte': 2}}, {'b': {'$lte': 1}, 'c': 4})

    assert q == {'a': 1, 'b': {'$lte': 1}, 'c': 4}
